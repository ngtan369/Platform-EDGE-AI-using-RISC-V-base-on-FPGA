...
module obi_to_axi #(
  /// The configuration of the OBI port (input port).
   parameter obi_pkg::obi_cfg_t ObiCfg      = ObiDefaultConfig, //obi_pkg::
//  parameter obi_pkg::obi_cfg_t ObiCfg = '0,
  ...
  /// AXI Burst Type (burst unused but may be required for IP compatibility)
   parameter int unsigned       AxiBurstType = BURST_INCR, //axi_pkg::
//  parameter int unsigned AxiBurstType = 1,
