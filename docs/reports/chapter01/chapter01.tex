...
\begin{itemize}
    ...
    \item \textbf{Pipeline huấn luyện và biên dịch mô hình:} Xây dựng pipeline huấn luyện mạng \texttt{vgg-tiny} INT8 trên Host PC (Google Colab + Tesla T4 GPU), kết hợp với trình biên dịch tự sinh layer table (\texttt{layer\_table.h}) và blob trọng số (\texttt{weights.bin}) -- cho phép thay đổi mô hình AI mà không cần tổng hợp lại RTL. Phạm vi đồ án sử dụng tập \textit{Cats vs Dogs} (Kaggle, 25{,}000 ảnh) cho task phân loại nhị phân; tập \textit{INRIA Person} được dự kiến cho hướng phát triển mở rộng sang task phát hiện đối tượng (object detection).
...
\begin{itemize}
    ...
    \item \textbf{Phạm vi kịch bản ứng dụng (Use-case) và Dữ liệu:} Hệ thống được kiểm chứng thông qua bài toán cốt lõi của Thị giác máy tính (Computer Vision) tại biên là phân loại ảnh nhị phân (Binary Image Classification), cụ thể với tập dữ liệu \textit{Cats vs Dogs} (Microsoft Asirra, 25{,}000 ảnh) -- ảnh tĩnh đọc từ thẻ nhớ. Việc mở rộng sang phát hiện đối tượng (Object Detection -- Human Detection trên INRIA Person), xử lý video thời gian thực độ phân giải cao (HD/Full HD), hay các loại tín hiệu âm thanh/văn bản được dự kiến cho các pha phát triển sau.
...
\begin{itemize}
    ...
    \item \textbf{Chương 3. Các công trình nghiên cứu liên quan:} \\
    ...
    \item \textbf{Chương 4. Thiết kế kiến trúc hệ thống:} \\
    Mô tả chi tiết kiến trúc SoC dị thể (heterogeneous) gồm 3 tầng (HW/SW/App), bản đồ bộ nhớ (memory map) thống nhất giữa ARM và RISC-V, kiến trúc Dual-Port BRAM cho mô hình bộ nhớ Harvard, thiết kế bộ tăng tốc \texttt{conv\_cnn} v2.0 (line buffer + 9 PE MAC + requantize Q31), kiến trúc Tiny-VGG (5 conv $3\times3$, INT8) và luồng inference 4 pha (ARM prep $\rightarrow$ kick $\rightarrow$ RISC-V duyệt layers $\rightarrow$ postprocess).

    \item \textbf{Chương 5. Hiện thực và kiểm thử hệ thống:} \\
    Trình bày quá trình hiện thực hóa thiết kế: pipeline huấn luyện 8 bước trên Jupyter notebook, lượng tử hóa PTQ INT8 và emit \texttt{layer\_table.h} + \texttt{weights.bin}, hiện thực Conv-CNN v2.0 (7 module RTL), tinh chỉnh CV32E40P và cầu OBI$\to$AXI, firmware bare-metal C, host PYNQ runtime, tích hợp Vivado Block Design, và quy trình kiểm thử nhiều tầng (unit TB $\rightarrow$ IP TB $\rightarrow$ golden vector $\rightarrow$ on-board).

    \item \textbf{Chương 6. Kết quả:} \\
    Trình bày kết quả định lượng: độ chính xác Tiny-VGG trên \textit{Cats vs Dogs} (FP32/INT8), báo cáo tổng hợp Vivado (tài nguyên, timing, công suất), phân tích latency theo layer ở 100~MHz, và bảng so sánh đa chiều với các framework liên quan.
