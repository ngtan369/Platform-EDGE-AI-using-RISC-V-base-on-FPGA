...
\begin{itemize}
    \item \textbf{Ứng dụng kiểm chứng (Use-case):} Hiện tại, tầng này thực thi cấu trúc mạng \textbf{Tiny-VGG} (5 lớp Conv $3\times3$ stride-1 valid padding, lượng tử hóa INT8) để giải quyết bài toán phân loại nhị phân (binary classification) trên tập dữ liệu \textit{Cats vs Dogs} (Microsoft Asirra, 25{,}000 ảnh tải qua Kaggle). Việc lựa chọn Tiny-VGG thay vì các kiến trúc lớn hơn (MobileNet, ResNet) xuất phát từ ràng buộc của bộ tăng tốc Conv-CNN v2.0 hiện tại -- chỉ hỗ trợ Conv $3\times3$ stride-1 valid padding, chưa hỗ trợ depthwise / batch normalization / residual connection. Đây là đánh đổi có chủ đích: hi sinh phạm vi mô hình để đạt được toàn quyền kiểm soát pipeline phần cứng (xem Chương~\ref{chp:03}).
    \item \textbf{Tính linh hoạt trong cùng miền ứng dụng (Domain-Specific Flexibility):} Nhờ thiết kế phân lớp, người dùng có thể dễ dàng chuyển đổi hệ thống sang các bài toán phân loại nhị phân khác trong cùng lĩnh vực thị giác máy tính (ví dụ: nhận diện biển báo giao thông, phân loại khiếm khuyết sản phẩm) chỉ bằng cách thay tập dữ liệu huấn luyện, sinh lại \texttt{layer\_table.h} + \texttt{weights.bin}, và recompile firmware -- hoàn toàn không cần can thiệp hay tổng hợp lại kiến trúc vật lý ở Layer 1. Việc mở rộng sang phát hiện đối tượng (object detection) hoặc các kiến trúc lớn hơn được dự kiến trong các pha mở rộng (xem Chương~\ref{chp:conclusion}).
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Lõi vi xử lý mềm RISC-V (CV32E40P):} Điều phối luồng thực thi thuật toán AI (kế thừa từ hệ sinh thái PULP của OpenHW Group, không sử dụng toàn bộ Pulpissimo subsystem).
        \item \textbf{Bộ tăng tốc phần cứng (Conv-CNN v2.0):} Thực hiện các phép toán tích chập $3\times3$ INT8 chuyên dụng.
        \item \textbf{Cơ chế truy cập bộ nhớ trực tiếp (AXI-DMA):} Sử dụng Xilinx AXI Direct Memory Access IP để di chuyển dữ liệu ảnh / trọng số / OFM giữa DDR4 và bộ tăng tốc mà không làm gián đoạn CPU.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{AXI Port QoS:} Cổng S\_AXI\_HP0 được ưu tiên mức Quality of Service (QoS) cao để đảm bảo AXI-DMA không bị nghẽn khi ghi dữ liệu ảnh, tránh hiện tượng rớt khung hình (frame drop).
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textit{Nhiệm vụ:} Cho phép các Master bên phía FPGA (cụ thể là AXI-DMA) truy cập trực tiếp vào RAM DDR4.
...
Đây là tuyến giao tiếp từ \textbf{Master (ARM Cortex-A53)} đến các \textbf{Slave trên PL} (RISC-V \texttt{riscv\_top}, \texttt{conv\_cnn\_0}, BRAM controllers).
\begin{itemize}
...
Đây là tuyến giao tiếp từ \textbf{Master (AXI-DMA Controller)} đến \textbf{Slave (DDR4 Controller qua S\_AXI\_HPC0\_FPD)}.
\begin{itemize}
    \item \textbf{Giao thức:} AXI4-Full 128-bit.
    \item \textbf{Mục đích:} Tối ưu hóa băng thông (Throughput).
    \item \textbf{Chức năng:} Hỗ trợ cơ chế \textit{Burst Transaction}, cho phép AXI-DMA đọc/ghi liên tiếp tới 256 gói dữ liệu chỉ với một địa chỉ bắt đầu. Điều này cực kỳ quan trọng để cung cấp đủ băng thông pixel/trọng số cho bộ tăng tốc Conv-CNN, đặc biệt khi mở rộng lên các kiến trúc CNN sâu hơn ở các pha phát triển tiếp theo.
\end{itemize}
...
\subsection{Cấu hình Nhân Vi xử lý CV32E40P}
\label{ssec:riscv_config}
Lõi RISC-V được lấy từ submodule \texttt{cv32e40p} của OpenHW Group, sau đó được tham số hóa qua các \texttt{define} trước khi tổng hợp:

\begin{table}[h]
\centering
\caption{Thông số cấu hình RISC-V Soft-core sử dụng trong đề tài.}
\label{tab:riscv_config}
\begin{tabular}{|l|l|p{6.5cm}|}
\hline
\textbf{Tham số} & \textbf{Giá trị} & \textbf{Lý do thiết kế} \\ \hline
Architecture       & RV32IMC                & Tập lệnh cơ bản + nhân/chia (M) + nén (C). Đủ cho vai trò orchestrator (lập trình DMA + đọc layer table). \\ \hline
Pipeline Stages    & 4                      & Cân bằng giữa Fmax và độ trễ rẽ nhánh. \\ \hline
FPU                & Disabled               & Toàn bộ tính toán đã lượng tử hóa INT8; bỏ FPU tiết kiệm $\sim$3000 LUT + 5 DSP. \\ \hline
COREV\_PULP        & Disabled               & Bỏ tập lệnh mở rộng Xpulp (HW loop, SIMD) -- không cần thiết cho orchestrator, đồng thời tránh phụ thuộc vào toolchain GCC tùy biến. \\ \hline
Instruction memory & I-BRAM 64 KB           & Đủ chứa firmware + LAYERS[] (compile-in). \\ \hline
Data memory        & D-BRAM 256 KB          & Chứa \texttt{.data}, \texttt{.bss}, stack, vùng handshake với ARM. \\ \hline
Boot Address       & \texttt{0x0000\_0000}  & Đầu I-BRAM, do ARM nạp \texttt{firmware.bin} qua Port B trước khi release reset. \\ \hline
\end{tabular}
\end{table}

\subsection{Kiến trúc bộ nhớ và Interconnect}
\label{ssec:bram_interconnect}
Hệ thống sử dụng cặp BRAM dual-port (RAMB36 trên Ultrascale+) làm core memory cho RISC-V, kết nối với 2 master (RISC-V và ARM) qua các \texttt{axi\_bram\_ctrl}. Đây là điểm khác biệt quan trọng so với kiến trúc TCDM 16-bank của PULPissimo: thay vì tối ưu cho nhiều master tranh chấp đồng thời, đề tài chỉ có 2 master và lựa chọn 2-port BRAM tự nhiên hỗ trợ truy cập song song không xung đột.

\begin{itemize}
    \item \textbf{I-BRAM 64 KB} -- Port A nối với \texttt{m\_axi\_instr} của RISC-V cho instruction fetch; Port B nối với ARM (qua HPM0\_FPD) cho phép ARM nạp/reload \texttt{firmware.bin} mà không cần halt RISC-V.
    \item \textbf{D-BRAM 256 KB} -- Port A nối với \texttt{m\_axi\_data} của RISC-V; Port B nối với ARM (qua HPM1\_FPD). Vùng đầu của D-BRAM dùng làm shared region với các thanh ghi handshake \texttt{CMD\_FROM\_ARM}, \texttt{STATUS\_TO\_ARM}, \texttt{IFM\_PHYS\_ADDR}, \texttt{OFM\_PHYS\_ADDR}, \texttt{WEIGHT\_BASE} (xem mục~\ref{sec:handshake_regs}).
\end{itemize}

Toàn bộ peripheral (AXI DMA, conv\_cnn S00\_AXI, BRAM controllers) được kết nối tới cả ARM và RISC-V qua một AXI SmartConnect chung, đảm bảo cả hai master cùng thấy slave ở chung một địa chỉ logic dải \texttt{0xB0xx\_xxxx}.

\subsection{Cơ chế DMA truy cập DDR}
\label{ssec:dma_design}
Đề tài sử dụng \texttt{axi\_dma\_0} (Xilinx AXI Direct Memory Access IP, Scatter-Gather disabled) thay vì thiết kế lại từ đầu như PULPissimo uDMA:

\begin{itemize}
    \item \textbf{Hai kênh độc lập:} MM2S (Memory-Mapped to Stream) đọc IFM/weights từ DDR và đẩy vào \texttt{conv\_cnn\_0/S00\_AXIS}; S2MM (Stream to Memory-Mapped) ghi OFM từ \texttt{conv\_cnn\_0/M00\_AXIS} ngược về DDR.
    \item \textbf{Cache coherency:} Cả hai kênh đi qua \texttt{S\_AXI\_HPC0\_FPD} (cấu hình coherent), tránh nhu cầu \texttt{flush}/\texttt{invalidate} cache thủ công bên ARM.
    \item \textbf{Cơ chế ping-pong DDR:} Firmware RISC-V duy trì hai vùng đệm DDR (\texttt{buf\_a}, \texttt{buf\_b}); layer chẵn đọc \texttt{buf\_a} ghi \texttt{buf\_b}, layer lẻ đảo lại. Cách này thay thế cho double-buffer ping-pong nội bộ uDMA của PULPissimo, nhưng không tốn BRAM nội (chỉ tốn DDR vốn dồi dào).
\end{itemize}
...
Để đáp ứng yêu cầu tính toán hiệu quả các phép tích chập của mạng \texttt{Tiny-VGG} (5 lớp Conv $3\times3$ INT8), nhóm thực hiện thiết kế một bộ tăng tốc phần cứng chuyên dụng (Domain-Specific Architecture -- DSA) tên là \texttt{conv\_cnn} v2.0, được tích hợp trong miền Logic khả trình (PL). Kiến trúc được thiết kế tổng quát ở mức tham số (\texttt{MAX\_WIDTH}, \texttt{MAX\_CIN}, \texttt{MAX\_COUT} trong package \texttt{conv\_pkg.sv}), cho phép scale lên các mạng lớn hơn (VGG11, ResNet18) ở các pha mở rộng mà không cần thay đổi datapath cốt lõi.
...
\begin{itemize}
    \item \textbf{AXI4-Stream Slave:} Nhận dòng pixel ảnh đầu vào từ AXI-DMA hoặc trực tiếp từ bộ nhớ đệm.
...
\begin{itemize}
    ...
    \item \textbf{Số tham số nhỏ:} Tổng cộng $\approx 25{,}700$ tham số INT8 (chi tiết per-layer dimensions trong bảng~\ref{tab:vgg_tiny_layers}), dung lượng weights blob khoảng 25 KB -- nằm gọn trong bộ nhớ DDR và dễ quản lý.
\end{itemize}
...
\begin{itemize}
    ...
    (Output-Height-Width-Input) để đảm bảo khi AXI-DMA đọc 16 bytes (128-bit) sẽ thu được 
