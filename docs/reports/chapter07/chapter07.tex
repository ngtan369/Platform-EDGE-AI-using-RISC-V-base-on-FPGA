...
\begin{itemize}
    \item \textbf{Phạm vi op còn hạn chế:} Conv-CNN v2.0 hiện chỉ hỗ trợ Conv 3$\times$3 stride~1 với padding \texttt{valid} và activation ReLU/None. Chưa hỗ trợ Conv 1$\times$1, depthwise separable, pooling, batch normalization, padding \texttt{same}, hay residual add. Điều này giới hạn các mô hình deploy được hiện tại ở mức Tiny-VGG; các kiến trúc phổ biến hơn như VGG11, ResNet18, MobileNet chưa thể chạy trực tiếp trên bo.
    \item \textbf{Per-tensor quantization:} TFLite hỗ trợ per-axis quantization (mỗi output channel có scale/zero\_point riêng), giúp giảm rớt accuracy thêm 1--2\% trên mạng sâu. Đề tài hiện dùng per-tensor (lấy \texttt{scales[0]}) để đơn giản hóa RTL requantize. Trên Tiny-VGG (5 layer), tác động không đáng kể (INT8 ngang FP32) nhưng sẽ giới hạn khi scale lên VGG11/ResNet18.
    ...
    \item \textbf{Bring-up host PYNQ chưa hoàn tất:} Tầng \texttt{host/edge\_ai/} (overlay wrapper, preprocess, postprocess) và notebook \texttt{inference\_demo.ipynb} đang được phát triển. Vì vậy các phép đo end-to-end thực tế trên KV260 (latency thực, FPS, on-board accuracy, hardware-in-the-loop verification) sẽ được bổ sung ở phiên bản cuối của báo cáo.
