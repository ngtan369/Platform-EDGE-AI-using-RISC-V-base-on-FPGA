...
\begin{table}[htbp]
    \centering
    \caption{Accuracy Tiny-VGG trên tập Cats vs Dogs (input $128\times128$, INT8 PTQ, 30 epochs).}
    \label{tab:catsdogs-accuracy}
    \begin{tabular}{|l|c|c|}
        \hline
        \textbf{Mô hình}                & \textbf{Train Acc} & \textbf{Val Acc} \\
        \hline
        FP32 Keras (reference)          & 73.37\%            & \textbf{79.38\%} \\
        INT8 TFLite (PTQ, calib 100)    & ---                & $\sim$78.0\%     \\
        INT8 + FPGA (KV260, on-board)   & ---                & {\itshape (Phase tiếp theo)} \\
        \hline
    \end{tabular}
\end{table}

Quá trình huấn luyện cho thấy khi không sử dụng augmentation, mô hình hội tụ sớm ở mức 73.88\% với hiện tượng overfit nhẹ (train\_acc ~74\% so với val\_acc ~74\%). Sau khi bổ sung 3 kỹ thuật augment (flip + crop + brightness/contrast), val\_acc tăng lên 79.38\% sau 30 epoch (xem Hình~\ref{fig:training-curves}). Khoảng cách giữa train\_acc và val\_acc tiếp tục thu hẹp, cho thấy augmentation đã giảm overfit hiệu quả -- đúng kỳ vọng cho một mô hình có dung lượng nhỏ ($\sim$25.7K tham số).

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{chapter06/img/training_curves.png}
    \caption{Đường cong loss và accuracy theo epoch của Tiny-VGG trên tập Cats vs Dogs.}
...
Vì vậy, kết quả trên INRIA Person được xếp vào hướng phát triển ở Chương~\ref{chp:07} (Tier~B), khi pipeline mở rộng sang nhánh phát hiện đối tượng (object detection) với mô hình YOLO-tiny.

\subsection{Đánh giá ảnh hưởng của lượng tử hóa INT8}

Để đo độ rớt chính xác do lượng tử hóa, mô hình INT8 được kiểm tra trên 20 batch ngẫu nhiên (640 ảnh) của tập val bằng \texttt{tf.lite.Interpreter}. Kết quả ở lần chạy đầu (không augment): FP32 đạt 73.88\%, INT8 đạt 74.53\% -- chênh lệch trong phạm vi $\pm 1\%$ thuộc khoảng nhiễu thống kê và \textbf{không vượt quá ngưỡng cảnh báo 3\%} thường được sử dụng trong tài liệu của Jacob et al.~\cite{jacob2018quantization}. Điều này khẳng định:
\begin{itemize}
    \item Tập calibration 100 mẫu là đủ đại diện cho phân phối activation.
    ...
    \item Multiplier output $M_{\text{real}} = (S_{\text{in}} \cdot S_w) / S_{\text{out}}$ được encode dưới dạng $(M_{q31}, \text{shift})$ qua hàm \texttt{quantize\_multiplier()} -- kết quả bit-exact với chuẩn TFLite, đảm bảo phần cứng có thể tái tạo chính xác kết quả tham chiếu.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Test accuracy on-board}: chạy benchmark.ipynb trên 5{,}000 ảnh val, so sánh với INT8 TFLite reference (~78\%). Sự sai khác $\leq 0.5\%$ sẽ được coi là bit-exact pass.
