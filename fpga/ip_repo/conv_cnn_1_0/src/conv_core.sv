...
controller_fsm u_fsm (
        ...
        .num_cout     (10'd1),         // T?m g�n 1 k�nh �?u ra (ho?c n?i t? ARM xu?ng)
        .width        (active_width),  // N?i width v�o
        .height       (active_width),  // Gi? s? ?nh vu�ng, n?i chung v�o active_width
        .num_cin      (num_cin),
        .acc_clear    (acc_clear),
        .mac_en       (mac_en),
...
// =====================================
    // 6. C�y C?ng D?n (Adder Tree) & �p Ki?u (Requantize/ReLU)
    // =====================================
    reg signed [31:0] total_sum;
    reg [7:0] conv_relu_out;   // D�y trung gian ch?a k?t qu? ReLU
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (bias_relu_en) begin
                ...
                // H�m ReLU v� �p v? INT8 l�u v�o bi?n t?m 'conv_relu_out'
